...
async def _ensure_sqlite_schema() -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("CREATE INDEX IF NOT EXISTS idx_bot_visits_visited_at ON bot_visits (visited_at)")
        await db.execute("CREATE INDEX IF NOT EXISTS idx_bot_visits_user_id ON bot_visits (user_id)")
        await db.commit()
...
async def _ensure_postgres_schema() -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_bot_visits_visited_at ON bot_visits (visited_at)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_bot_visits_user_id ON bot_visits (user_id)")
        ts = _now_iso()
    ...
    if using_postgres():
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute(
                    """
                    INSERT INTO bot_visits (user_id, username, visited_at)
                    VALUES ($1, $2, $3)
                    """,
                    user_id,
                    uname,
...
async def _visit_counts_since(since_iso: str) -> dict[str, int]:
    row = await _fetchone(
        """
        SELECT COUNT(*) AS visits, COUNT(DISTINCT user_id) AS visitors
        FROM bot_visits
        WHERE visited_at >= ?
        """,
        (since_iso,),
...
async def bot_visit_stats() -> dict[str, dict[str, int]]:
    return {
        "day": await _visit_counts_since(_since_iso(1)),
        "week": await _visit_counts_since(_since_iso(7)),
        "month": await _visit_counts_since(_since_iso(30)),
    }
